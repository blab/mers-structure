...
This manifests as different rates of coalescence with camel viruses showing a scaled effective population size $N_e \tau$ of \tbc{XXX (95\% HPD: XXX--XXX)} and human viruses showing a scaled effective population of \tbc{XXX (95\% HPD: XXX--XXX)}.
...
\begin{figure}[h]
 \centering
  ...
  \tbc{The previous two sentences (``Some of the...'') seem unnecessary and confusingly written. I would just drop them.}
...
\tbc{I think this is the best place to discuss the lack of correlation between date and size of human cluster. This can be referenced later on in Discussion when talking about pandemic potential. Also, seems relevant to look at seasonality in size of cluster here, yes? This would get at whether there are seasonal factors on the human side.}
...
Model averaging would suggest plausible $R_0$ values between \tbc{XXX} and \tbc{XXX}.
...
Model averaging would suggest plausible numbers of introductions between \tbc{XXX} and \tbc{XXX}.
...
%Overall we advise extreme caution when interpreting branches in MERS-CoV phylogenies that are suspected of having spent any time in camels, especially when attempting to identify origins of human cases. %% a bit overblown 'extreme caution'? %TB: Agree to leave out this sentence.
...
% On the other hand, the largely clonal evolution of MERS-CoV in humans, despite limited diversity, should be more amenable to traditional phylogenetic approaches. %% But why won't recombination be happening in humans? Sure it won't be 2 divergent lineages.
...
\begin{figure}%
    ...
    Genomic fragment 1 represents nucleotide positions 1 to 21000 and genomic fragment 2 represents nucleotide positions 21001 to \tbc{XXX}.
    ...
    B) Phylogenies derived from genomic fragment 1 (left) and genomic fragment 2 (right), reduced to just the human tips.
  	...
  	While some of apparent incongruities are caused by having less data in some of the fragments, inconsistencies between topologies occur across internal branches inferred to be in camels, adding support to the idea that camels are often co-infected. %\lmc{I'm sure this is important, but I didn't get why. Referees might not either...}
    \tbc{I would favor just dropping the ``While some of apparent...'' sentence. Confusing and I don't see what it adds.}
...
Out of five runs, two chains converged to one stationary distribution, two to another and the last one to a third.
...
\tbc{This entire paragraph should be moved to Methods. Too much detail for Results. Below just say that ``Multiple MCMC chains showed similar patternso of $N_e \tau$ through time.''}
...
\tbc{This section seems like its reaching pretty far, but if keeping could mention that human seasonal coronaviruses undergo antigenic drift.}
...
\tbc{This seems way overly broad to me. I much preferred the Introductions take on genomic epi revealing important population structure. Just revise to keep this focused on pathogen transmission. Identification of reservoirs, source-sink, all very important.}
...
While there are clear indications that human cases of MERS are largely the result of sporadic spillover from camels, sequence data have not yet been fully utilised in estimating basic epidemiological parameters, such as $R_0$ or the number of zoonotic introductions of MERS-CoV into humans.
\tbc{This sentence (``While there...'') is overly like Intro.}
...
\tbc{I like the basic approach, this last segue could be cleaner however.}
...
\tbc{Perhaps close with discussion of evolution of host tropism? So far outbreaks have been self-limiting, but continued circulation in camels and continued spill-over into humans provides continued chances for evolution of a more human transmissible virus.}
...
\tbc{Mention what's going on with accession-less sequences.}
Fragments of some strains submitted to GenBank as separate accessions were assembled into a single sequence.
\tbc{This should be obvious in Table \ref{sequences}. Some strains should be multiple accessions. However, I don't see any.}
\tbc{What we the criteria for inclusion? How many bases were necessary to be considered a `genome'?}
...
\tbc{Leaving how many bases?}
\tbc{How were they aligned?}
...
MultiTypeTree module \citep{vaughan_efficient_2014} was used in BEAUti v2.4.3 \citep{bouckaert_beast_2014} to specify a structured coalescent model with two demes -- humans and camels.
...
\tbc{What was burn-in? How many steps in the MCMC? How was convergence assessed? How were chains combined?}
...
As a secondary analysis, we also considered a scenario where alignments split into two fragments (fragment 1 comprised of positions 1-21000, fragment 2 of positions 21000-onwards), with independent clocks, trees and migration rates, but shared substitution models and deme population sizes.
...
Due to the increased complexity of multitype tree parameter space 50\% of states from every analysis were discarded as burn-in for single fragment analyses and 20\% for two-fragment analyses.
