...
In the absence of other information often only sequence data can %\lmc{help}
...
% ``mugration'' approaches that simply infer and label the internal nodes of a phylogeny,
% please lets drop 'mugration'. Terrible word. Also I don't see why they are 'simpler'.
% structured population models possess the ability to estimate migration times. % So do Markov Jumps. \gdc{good point, yeah}
We use the posterior distribution of MERS-CoV introduction times of human clades observed in the MCC tree (Figure \ref{mcc}) recovered from structured coalescent to model seasonal variation in zoonotic transfer of viruses.
...
A) %Violin plots% %  (the plot type is irrelevant).
...
Our results suggest a large number of unobserved MERS primary cases.%\lmc{does the definition of ``index case'' allow for ``multiple''?} \gdc{not in epi literature I don't think}
...
We find little evidence that recombination will interfere with the inference of human outbreak clusters %beyond what could be expected of phylogenetic noise %% doesn't make sense to say that recombination is interfering as expected by phylogenetic noise.
...
Observed departures from strictly clonal evolution suggest that whilst recombination is an issue for inferring MERS-CoV phylogenies, its effects on the human side of MERS outbreaks is minimal, as expected.
...
%Overall we advise extreme caution when interpreting branches in MERS-CoV phylogenies that are suspected of having spent any time in camels, especially when attempting to identify origins of human cases. %% a bit overblown 'extreme caution'?
...
Severe Acute Respiratory Syndrome coronavirus, a Betacoronavirus like MERS-CoV, caused a serious epidemic in humans in 2003, with over 8000 cases and nearly 800 deaths.
...
We sample 174 cases from each simulation to represent sequencing of human MERS cases.%\lmc{is this implicitly a binomial sampling with $p = 1/2$?} \gdc{hypergeometric, since we probably don't resequence people}
