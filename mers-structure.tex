...
This manifests as different rates of coalescence with camel viruses showing a scaled effective population size $N_e \tau$ of 3.49 (95\% HPD: 2.71--4.38) and human viruses showing a scaled effective population of 0.24 (95\% HPD: 0.14--0.34).
...
Although we find evidence of seasonality in zoonotic spill over timing, no such relationship exists for sizes of human sequence clusters (\textit{i.e. post-spillover clades})(?).
...
Similarly, we do not observe any trend in human sequence cluster sizes over time, suggesting that MERS-CoV outbreaks in humans are neither growing nor shrinking in size.
This is not surprising either, since MERS-CoV is a camel virus that experiences little to no selective pressure to improve transmissibility between humans.
...
On a smaller scale population structure can unveil important details about transmission patterns, such as identifying reservoirs and understanding spill over trends and risk, much as we have done here.
...
Although MERS-CoV seems to cause self-limiting outbreaks in humans with no evidence of worsening outbreaks over time, sustained evolution of the virus in the reservoir and flow of viral lineages into humans does not eliminate the possibility that a more transmissible variant of MERS-CoV will emerge.
...
All analyses were run across for 200 million states across ten independent Markov chains (MCMC runs).
%\tbc{What was burn-in? How many steps in the MCMC? How was convergence assessed? How were chains combined?}
Due to the increased complexity of multitype tree parameter space 50\% of states from every analysis were discarded as burn-in, convergence assessed in Tracer v1.6 and states combined using LogCombiner distributed with BEAST v2.4.3 \citep{bouckaert_beast_2014}.
...
For these analyses two chains did not converge.
