...
We employ state-of-the-art structured coalescent models to show that long-term MERS-CoV evolution occurs exclusively in camels, whereas humans act as a transient, and ultimately terminal host.
...
Monte Carlo simulations indicate $R_{0}$ for MERS-CoV is likely to be below 1.0, with biased sequencing and numbers of zoonotic transmissions numbering in the hundreds.
...
\begin{figure}[h]
\centering
	\includegraphics[width=0.65\textwidth]{figures/mers_method.pdf}
...
A simulations is considered to match if its sequence cluster size mean and standard deviation fell within the 95\% highest posterior density interval of observed MERS-CoV sequence clusters.
$R_{0}$ values that ultimately generate data matching empirical observations, as well as associated numbers of `introductions' are summarised in Figure \ref{ABC}.
	}
	\label{method}
\end{figure}
