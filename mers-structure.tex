...
It is possible that MERS-CoV sweeps through each new camel generation once critical mass of susceptibles is reached, leading to a sharp rise in incidence of the virus in camels that leads to increased force of infection sparking human outbreaks.
Strong influx of susceptibles and subequent sweeping outbreaks in camels may explain evidence of widespread exposure to MERS-CoV in camels from seroepidemiology \citep{muller_2014,corman_antibodies_2014,chu_2014,reusken_2013,reusken_2014}.
...
Structured coalescent approaches clearly show humans to be a terminal host \tbc{again, I don't know if this is the right term or not} for MERS-CoV, implying poor transmissibility.
However, we wanted to translate this observation into the basic reproductive number, $R_{0}$, which is more familiar to epidemiologists and provides expectations of epidemic behavior.
The parameter $R_{0}$, along with the degree of heterogeneity in tranmission quantified by dispersion parameter $\omega$, can be used to determine the distribution of total cases that result from a single introduction event into the human population (Equation \ref{clusterLikelihood}).
We estimate $R_{0}$ along with other relevant parameters via approximate Bayesian computation (ABC) in two steps.
We simulate case counts across multiple outbreaks totalling 2000 cases using Equation \ref{clusterLikelihood} with a fixed dispersion parameter $\omega$ and then subsample using a multivariate hypergeometric distribution from each case cluster to simulate sequencing of a fraction of cases.
...
We find that observed phylogenetic patterns sequence clustering strongly support $R_{0}$ below 1.0 (middle panels in Figure \ref{ABC}).
...
Under unbiased sequencing only 0.6\% (2282 out of 363 000) of simulations fit our phylogenetic observations, while 2.7\% and 2.7\% of simulations fit for bias levels of 2.0 and 3.0, respectively.
This is apparent in the heights of the histograms in Figure \ref{ABC}.
This is equivalent to a higher marginal likelihood for bias levels of 2.0 and 3.0 relative to bias of 1.0 (10\% support for bias level 1.0, 45\% support for bias level 2.0, and 45\% support for bias level 3.0).
...
Overall our analyses indicate that MERS-CoV is poorly suited for human-to-human transmission, with an estimated $R_{0}<1.0$ and sequence sampling wholly unrepresentative of viral diversity found in humans.
...
ABC indicates $R_{0}$ for MERS-CoV is likely to be below 1.0, with biased sequencing and numbers of zoonotic transmissions numbering in the hundreds.
\tbc{Not at all essential, but I can imagine that an initial panel A showing the logic of the two step ABC process would do a whole lot to eleviate confusion. I'm picturing a set of tables numbered with case counts in a top row and then a bottom row with sequence counts. This could also work better as a separate figure referenced from the Methods.}
	}
	\label{ABC}
\end{figure}
...
Recombination has been shown to occur in all genera of coronaviruses, including MERS-CoV \tbc{needs citations}.
