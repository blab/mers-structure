...
\begin{algorithm}[H]
 ...
 Draw from $S_{i}$ from a hypergeometric distribution with $C_{i}$ successes, $\sum_{i=1}^{K} C_{i}-C_{i}$ failures after $M$ trials\;
 \While{$i < K$}{
  $i = i + 1$\;
  $M = M - S_{i-1}$\;
  Compute the probability mass function (pmf) for all possible values of $S_i$, $\boldsymbol p = [p(0)^{\text{bias}}, p(1), \ldots, p(C_{i})^{\text{bias}}] \times (\sum_i p_i^{\text{bias}}) ^{-1}$, where $p(\cdot)$  is the pmf for a hypergeometric distribution with $C_{i}$ successes, $\sum_{i=1}^{K} C_{i}-C_{i}$ failures after $M$ trials\;
